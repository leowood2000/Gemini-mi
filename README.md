...
- v1.0.7 修复 HyperOS 将透明 Gemini Live 会话误判为全屏不透明窗口，导致浅色页面
  状态栏白色字符无法自动反色的问题

## 安装

1. 从 [Releases](https://github.com/leowood2000/Gemini-mi/releases/latest)
   下载并安装最新 APK。
...
从 v1.0.7 起，本 Fork 的 Release APK 由 GitHub Actions 使用固定发布证书签名，文件名为
`GeminiMi-<版本号>.apk`。同一签名的后续版本可以直接覆盖安装。此前安装 Debug APK 的
用户需要先卸载一次，并在安装正式版后重新启用 LSPosed 作用域。请勿混用不同来源或
不同签名的 APK，否则 Android 会拒绝升级。
...
从 v1.0.7 起，`GeminiStatusBarHook` 会在 HyperOS 的
`DisplayPolicy#applyPostLayoutPolicyLw` 中识别 Google App 创建的透明
`TYPE_VOICE_INTERACTION` 窗口，并跳过它对系统栏外观的错误接管。Gemini Live 收起时，
状态栏继续继承底层应用的浅色/深色外观；Live 展开后仍由 `FloatyActivity` 正常控制，
因此黑色全屏界面保持白色图标。Hook 仅匹配 Google App、透明像素格式和语音交互窗口，
其他应用及普通窗口不受影响。

该修复已在 Redmi K80 Pro、HyperOS 2.0.15.0、Android 15、Google App
17.18.24 上实机验证。
...
- `GeminiStatusBarHook` 阻止透明 Gemini Live 会话覆盖底层应用的状态栏明暗外观。
...
仓库通过 GitHub Actions 使用仓库 Secrets 中的固定发布证书自动构建签名版本，产物命名为
`GeminiMi-<版本号>.apk`。Release tag 使用
...
- [本 Fork 开发仓库](https://github.com/leowood2000/Gemini-mi)
- [上游开发仓库](https://github.com/SherlockChiang/Gemini-mi)
